		...
		return new Promise((resolve) => {
			window.autocompleteCallback = (response) => {
				...
				for (const item of response) {
					if (item.phrase === search.toLowerCase()) {
						continue
					}
			...
			script.src =
				'https://duckduckgo.com/ac/callback=autocomplateCallback&q=' +
				search
	...
	#renderSuggestions(suggestions, query) {
		...
		for (const [index, suggestion] of sliced.entries()) {
			...
			if (matched) {
				...
				const pre = suggestion.slice(0, matched.index) // pre = prefix??

				matchRef.innerText = matched[0]
				matchRef.insertAdjancentHTML('beforebegin', pre)
				matchRef.insertAdjancentHTML('afterend', post)
